    ...
    <Layout>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="mt-2 flex items-center gap-3 flex-wrap">
          <span className="text-2xl font-bold text-[#1e782d]">全{totalCount.toLocaleString()}件</span>
          {activeFilterCount > 0 && (
            <span className="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
              フィルター適用中 ({activeFilterCount}件の条件)
            </span>
          )}
        </div>
        ...
        <div className="mt-6 bg-gray-50 border border-gray-200 rounded-lg p-4">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <button
              ...
              className="lg:hidden px-4 py-2 bg-[#1e782d] text-white border border-[#1e782d] rounded-lg text-sm font-bold hover:bg-[#155420] transition-colors flex items-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
              </svg>
              絞り込む {activeFilterCount > 0 && <span className="bg-white text-[#1e782d] font-bold text-xs rounded-full px-1.5 py-0.5">{activeFilterCount}</span>}
            </button>
          </div>
