        ...
        <div className="mt-6 bg-gray-50 border border-gray-200 rounded-lg p-4">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <button
              ...
              className="lg:hidden px-4 py-2 bg-white border-2 border-[#1e782d] rounded-lg text-sm font-bold text-[#1e782d] hover:bg-[#f0f6f0] transition-colors flex items-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
              </svg>
              絞り込みフィルタ {activeFilterCount > 0 ? <span className="bg-[#1e782d] text-white rounded-full w-5 h-5 text-xs flex items-center justify-center">{activeFilterCount}</span> : <span className="text-xs text-gray-400">▼</span>}
            </button>
          </div>

          {/* フィルタセクション - 常にデスクトップ表示 */}
          <div className={`mt-4 pt-4 border-t border-gray-200 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 ${!isFilterExpanded ? "hidden lg:grid" : "grid"}`}>
            {/* 価格帯フィルタ */}
            <div>
