...
export default function Home({ featuredGyms, totalCount, regions, topCities, topStations }: HomeProps) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    router.push(`/all/${kw ? `?kw=${encodeURIComponent(kw)}` : ""}`);
  };
  ...
  return (
    <Layout>
      ...
      {/* Hero */}
      <section className="bg-gradient-to-br from-[#e8f5e9] to-white py-12 md:py-20">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          {/* 検索フォーム */}
          <form onSubmit={handleSearch} className="mt-8 max-w-xl mx-auto flex gap-2 bg-white rounded-xl p-2 shadow-md border border-gray-200">
            <input
              type="text"
              value={searchKw}
              onChange={e => setSearchKw(e.target.value)}
              placeholder="エリア・ジム名・駅名で検索"
              className="flex-1 px-4 py-2.5 text-sm text-gray-800 bg-transparent focus:outline-none"
            />
            <button
              type="submit"
              className="bg-[#1e782d] text-white font-bold px-6 py-2.5 rounded-lg text-sm hover:bg-[#155f22] transition-colors flex-shrink-0"
            >
              検索
            </button>
          </form>
          <div className="mt-3 flex flex-wrap justify-center gap-2">
            {["渋谷", "新宿", "銀座", "女性専用", "無料体験あり"].map(tag => (
              <NextLink
                key={tag}
                href={`/all/?kw=${encodeURIComponent(tag)}`}
                className="text-xs px-3 py-1.5 bg-white border border-gray-200 rounded-full text-gray-600 hover:border-[#1e782d] hover:text-[#1e782d] transition-colors no-underline"
              >
                {tag}
              </NextLink>
            ))}
