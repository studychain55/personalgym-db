...
export default function Home({ featuredGyms, totalCount, regions, topCities, topStations }: HomeProps) {
  ...
  const handleSearch = useCallback((e: React.FormEvent) => {
    ...
    router.push(kw ? `/all/?kw=${encodeURIComponent(kw)}` : "/all/");
  }, [searchKw, router]);
      ...
      <section className="bg-gradient-to-br from-[#FFF3ED] to-white py-12 md:py-20">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <p className="mt-4 text-lg text-gray-600">
            全国<span className="font-bold text-gray-900">{totalCount > 0 ? `${totalCount.toLocaleString()}件以上` : ""}</span>のパーソナルジムを料金・口コミ・特徴で徹底比較
          </p>
          <form onSubmit={handleSearch} className="mt-8 max-w-xl mx-auto flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-3 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1e782d] bg-white"
            />
            <button type="submit" className="bg-[#1e782d] text-white font-bold px-6 py-3 rounded-lg hover:bg-[#155420] transition-colors flex-shrink-0">
              検索
            </button>
          </form>
          <div className="mt-4 flex flex-wrap justify-center gap-2">
            {["無料体験あり", "女性専用", "駅近", "初心者向け"].map(tag => (
              <NextLink key={tag} href={`/all/?kw=${encodeURIComponent(tag)}`}
