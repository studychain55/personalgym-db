    ...
    <Layout>
      ...
      <section className="bg-gradient-to-br from-[#FFF3ED] to-white py-12 md:py-20">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <div className="mt-8">
            <NextLink
              href="/all/"
              className="inline-block bg-[#1e782d] text-white font-bold px-8 py-3 rounded-lg hover:bg-[#E55E2F] transition-colors no-underline shadow-lg"
            >
              ジム一覧を見る →
            </NextLink>
          </div>
          <p className="text-center text-sm text-gray-500 mt-3">全国<span className="font-bold text-gray-800">{totalCount.toLocaleString()}</span>件掲載中 · 毎日更新</p>
        </div>
      </section>
      ...
      <section className="bg-gradient-to-r from-orange-100 to-red-50 py-12">
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-8 text-center">数字で見るパーソナルジム</h2>
      ...
      <section className="bg-white py-12">
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-6 text-center">よくある質問</h2>
          <div className="max-w-4xl mx-auto space-y-4">
            {[
              { q: "パーソナルジムに通う費用はどのくらいですか？", a: "パーソナルジムの費用はコースにより異なりますが、1～3ヶ月のダイエットコースで15万～60万円、都度払い（1回）で8,000～15,000円が一般的な相場です。無料体験レッスンを活用して自分に合ったジムを選びましょう。" },
              { q: "パーソナルジムはどのくらいの期間通えば効果が出ますか？", a: "個人差はありますが、週2回以上のトレーニングで2～3ヶ月が一つの目安です。食事管理も合わせて行うことで効果が出やすくなります。多くのパーソナルジムでは2～3ヶ月の短期集中コースを提供しています。" },
              { q: "パーソナルジムと普通のジムの違いは何ですか？", a: "パーソナルジムは専属トレーナーが個人の目標・体型・体力に合わせてプログラムを設計し、マンツーマンで指導します。普通のジムに比べて費用は高めですが、効率的に目標達成できます。食事管理・栄養指導が含まれるコースも多いです。" },
              { q: "初心者でもパーソナルジムに通えますか？", a: "はい、パーソナルジムは運動未経験・初心者の方こそ活用いただける施設です。トレーナーが基礎から丁寧に指導するため、正しいフォームを身につけながら安全にトレーニングを始められます。" },
              { q: "パーソナルジムの選び方のポイントは？", a: "①目的（ダイエット・筋力アップなど）に合ったコースがあるか、②トレーナーの資格・実績、③立地・通いやすさ、④費用と契約条件の透明性、⑤無料体験の有無を確認しましょう。複数のジムを体験してから選ぶことをおすすめします。" },
            ].map((item, i) => (
              <details key={i} className="border border-gray-200 rounded-lg p-4 cursor-pointer group">
                ...
                <p className="text-gray-700 mt-3 ml-4">{item.a}</p>
              </details>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {regions.map((region) => (
              <div key={region.id} className="bg-white rounded-lg p-5 border border-gray-200">
                <h3 className="font-bold text-gray-800 mb-3 pb-2 border-b border-gray-100">{region.name}</h3>
                <div className="flex flex-wrap gap-2">
                  {region.prefectures.map((pref) => (
                    <NextLink key={pref.id} href={`/prefecture/${pref.slug}/`}
                      className="text-sm text-gray-600 hover:text-[#1e782d] no-underline transition-colors">
                      {pref.title}
                      {pref.gym_count > 0 && <span className="text-xs text-gray-400 ml-0.5">({pref.gym_count})</span>}
                    </NextLink>
                  ))}
                </div>
              </div>
        ...
        <section className="py-12">
          <div className="max-w-6xl mx-auto px-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
              {topCities.map((city) => (
                <NextLink key={city.slug} href={`/c-${city.slug}/`}
      ...
      <section className="bg-[#f0f6f0] py-12">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { href: "/column/gym-beginner/", tag: "初心者向け", title: "パーソナルジム初心者ガイド｜始め方・準備すること", desc: "パーソナルジムが初めての方へ。始める前に必要な準備をまとめました。" },
              { href: "/column/gym-cost/", tag: "費用", title: "パーソナルジムの料金相場を解説", desc: "パーソナルジムの料金体系を徹底解説。相場費用をまとめた比較表。" },
              { href: "/column/diet-gym/", tag: "ダイエット", title: "ダイエットにパーソナルジムをおすすめする理由", desc: "ダイエット成功率が高いパーソナルジムの秘訣を解説します。" },
            ].map((col) => (
              <NextLink key={col.href} href={col.href}
                ...
                <div className="p-6 flex flex-col h-full">
                  <div className="text-xs font-semibold text-[#1e782d] bg-[#e9f2ea] px-3 py-1 rounded-full inline-block mb-3 w-fit">{col.tag}</div>
                  <h3 className="text-lg font-bold text-gray-900 mb-2 flex-grow line-clamp-2 hover:text-[#1e782d] transition-colors">{col.title}</h3>
                  <p className="text-sm text-gray-600 line-clamp-2 mb-4">{col.desc}</p>
