  ...
  return (
    <Layout>
      <SEO
        ...
        description="パーソナルジムDBへのお問い合わせはこちら。掲載・修正依頼など、お気軽にご連絡ください。"
        path="/contact/"
      />
      <div className="max-w-2xl mx-auto px-4 py-8">
        <Breadcrumb items={[{ label: "お問い合わせ" }]} />
        <h1 className="text-2xl font-bold text-gray-900 mt-4 mb-2">お問い合わせ</h1>
        <p className="text-gray-500 text-sm mb-6">掲載依頼・情報修正・その他のご質問はこちらからどうぞ。</p>

        {/* 信頼シグナル */}
        <div className="flex flex-wrap gap-4 mb-6">
          {["営業日2日以内に返信", "完全無料", "個人情報は安全に管理"].map((t) => (
        ...
        {status === "success" ? (
          <div className="bg-green-50 border border-green-200 rounded-xl p-8 text-center">
            ...
            <p className="text-[#1e782d] font-bold text-lg mb-1">送信が完了しました</p>
            <p className="text-gray-600 text-sm">営業日2日以内にご返信いたします。</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="掲載依頼・情報修正・その他ご質問をご記入ください"
                className="w-full border border-gray-300 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-[#1e782d] focus:ring-1 focus:ring-[#1e782d]"
              />
            </div>
            {status === "error" && (
              <p className="text-red-500 text-sm bg-red-50 px-3 py-2 rounded-lg">送信に失敗しました。もう一度お試しください。</p>
            )}
            <button
              type="submit"
            ...
            <p className="text-[10px] text-gray-400 text-center">
              送信いただいた情報はお問い合わせ対応にのみ使用します。
            </p>
          </form>
        )}

        <div className="mt-6 text-center text-sm text-gray-500">
          メールでのお問い合わせ: <a href="mailto:info@personalgym-db.jp" className="text-[#1e782d] hover:underline">info@personalgym-db.jp</a>
        </div>
      </div>
    </Layout>
  );
