...
export default function InquiryForm({ siteId, facilityTable, facilityId, facilityUid, facilityName }: Props) {
  ...
  return (
    <>
      <div className="mb-5 p-4 bg-green-50 border border-green-200 rounded-lg text-sm text-gray-700">
        <p className="font-bold text-[#1e782d] mb-1">お問い合わせは無料です</p>
        <p>通常1〜3営業日以内にご返信いたします。個人情報は<a href="/privacy/" className="underline text-[#1e782d]">プライバシーポリシー</a>に基づき適切に管理します。</p>
      </div>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <p className="text-xs text-gray-500 text-center mt-2">送信後のしつこい営業は一切ありません</p>
      </form>
    </>
  );
}
